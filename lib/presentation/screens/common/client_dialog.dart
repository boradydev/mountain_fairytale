...
class _ClientDialogState extends State<ClientDialog> {
  ...
  Future<void> _deleteClient() async {
    ...
    final confirmed = await ConfirmDialog.show(
      ...
      content: 'Вы действительно хотите удалить клиента «${client.name}»?',
      confirmText: 'Удалить',
      destructive: true,
    );
            ...
            return AppDropdown<SalesRepresentative>(
              ...
              onAdd: () async {
                ...
                if (result == true && mounted) {
                  // Получаем последнего добавленного представителя из списка провайдера
                  final lastAddedRep = provider.salesRepresentatives.first;

                  setState(() {
                    _selectedSalesRepId = lastAddedRep.id;
                  });
                }
              },
